...
#include "arquivos.h" //Biblioteca com as structs criadas pelo nosso grupo para operar o programa
...
int escreve_vetor_cliente_txt(){
    ...
    for(int i=0;i<vetor_tamanhos[1][1];i++){
        ver=fprintf(p_arquivo,"%d;%s;%lu;%s;%s;%d;%s;%s;%d;%lu;%s\n", vetor_cliente[i].codigo, vetor_cliente[i].nome, vetor_cliente[i].endereco.estado, vetor_cliente[i].endereco.cidade, vetor_cliente[i].endereco.cep, vetor_cliente[i].endereco.bairro, vetor_cliente[i].endereco.rua, vetor_cliente[i].endereco.numero, vetor_cliente[i].telefone, vetor_cliente[i].email);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return -1;
    ...
    for(int i=0;i<vetor_tamanhos[1][1];i++){
        ver=fprintf(p_arquivo_backup,"%d;%s;%lu;%s;%s;%d;%s;%s;%d;%lu;%s\n", vetor_cliente[i].codigo, vetor_cliente[i].nome, vetor_cliente[i].endereco.estado, vetor_cliente[i].endereco.cidade, vetor_cliente[i].endereco.cep, vetor_cliente[i].endereco.bairro, vetor_cliente[i].endereco.rua, vetor_cliente[i].endereco.numero, vetor_cliente[i].telefone, vetor_cliente[i].email);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return 0;
...
//-Concertar escrita em txt capenga
//-Criar leitura de arquivos em txt
