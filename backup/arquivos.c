...
#define qtdvet 11 //Refere-se principalmente a quantidade de vetores de Struct presentes no programa
...
5-vetor_servico
6-vetor_funcionario
7-vetor_ordemservico
8-caixa.transacoes
9-caixa.contas_pagar
10-caixa.contas_receber
...
OrdemServico *vetor_ordemservico;
...
int inicia_vetor_estruturas(){
    ...
    vetor_ordemservico=(OrdemServico*)calloc(10, sizeof(OrdemServico));
    caixa.contas_pagar=(ContaPagar*)calloc(10, sizeof(ContaPagar));
    caixa.contas_receber=(ContaReceber*)calloc(10, sizeof(ContaReceber));
    caixa.transacoes=(TransacaoCaixa*)calloc(10, sizeof(TransacaoCaixa));
    caixa.montante=0;
    caixa.num_contas_pagar=0;
    caixa.num_contas_receber=0;
    caixa.num_transacoes=0;
    if (vetor_cliente == NULL || vetor_veiculo == NULL || vetor_peca == NULL || vetor_fornecedor == NULL || vetor_servico == NULL || vetor_funcionario == NULL || vetor_ordemservico == NULL){
        return -1;
    }
    return 1;
}
...
void realoca_vetores(){
    ...
    vetor_ordemservico=(OrdemServico*)realoca_vetor(vetor_ordemservico, vetor_tamanhos[7][0], sizeof(OrdemServico), 7);
    return;
}
...
int escreve_vetor_cliente_txt(){
    ...
    for(int i=0;i<vetor_tamanhos[1][1];i++){
        ver=fprintf(p_arquivo_backup,"%d;%s;%ld;%s;%s;%d;%s;%s;%d;%ld;%s;%d\n", vetor_cliente[i].codigo, vetor_cliente[i].nome,vetor_cliente[i].cpf_cnpj, vetor_cliente[i].endereco.estado, vetor_cliente[i].endereco.cidade, vetor_cliente[i].endereco.cep, vetor_cliente[i].endereco.bairro, vetor_cliente[i].endereco.rua, vetor_cliente[i].endereco.numero, vetor_cliente[i].telefone, vetor_cliente[i].email, vetor_cliente[i].existe);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return 0;
...
int escreve_vetor_veiculo_txt(){
    ...
    for(int i=0;i<vetor_tamanhos[2][1];i++){
        ver=fprintf(p_arquivo_backup,"%s;%s;%s;%d;%s;%d;%d\n",vetor_veiculo[i].placa,vetor_veiculo[i].modelo,vetor_veiculo[i].marca,vetor_veiculo[i].anoFabricacao,vetor_veiculo[i].chassi,vetor_veiculo[i].proprietario, vetor_veiculo[i].existe);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return 0;
...
int escreve_vetor_peca_txt(){
    ...
    for(int i=0;i<vetor_tamanhos[3][1];i++){
        ver=fprintf(p_arquivo_backup,"%d;%s;%s;%s;%f;%f;%d;%d;%d\n",vetor_peca[i].codigo,vetor_peca[i].descricao,vetor_peca[i].fabricante,vetor_peca[i].fornecedor,vetor_peca[i].precoCusto,vetor_peca[i].precoVenda,vetor_peca[i].quantidadeEstoque,vetor_peca[i].estoqueMinimo, vetor_peca[i].existe);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return 0;
...
int escreve_vetor_servico_txt(){
    ...
    for(int i=0;i<vetor_tamanhos[5][1];i++){
        ver=fprintf(p_arquivo_backup,"%d;%s;%f;%f;%d\n",vetor_servico[i].codigo,vetor_servico[i].descricao,vetor_servico[i].preco,vetor_servico[i].comissao, vetor_servico[i].existe);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return 0;
...
int escreve_vetor_funcionario_txt(){
    ...
    for(int i=0;i<vetor_tamanhos[6][1];i++){
        ver=fprintf(p_arquivo_backup,"%s;%ld;%s;%f;%d\n",vetor_funcionario[i].nome, vetor_funcionario[i].cpf,vetor_funcionario[i].cargo,vetor_funcionario[i].salario, vetor_funcionario[i].existe);
        if(ver<0){
            fclose(p_arquivo);
            fclose(p_arquivo_backup);
            return -1;
...
int le_vetor_cliente_txt(int operacao){
    ...
    for(int i=0;i<vetor_tamanhos[1][1];i++){
        fgets(linha, sizeof(linha), p_arquivo);
        linha[strcspn(linha, "\n")] = '\0';
        if(sscanf(linha, "%d;%99[^;];%ld;%2[^;];%29[^;];%d;%29[^;];%49[^;];%d;%ld;%49[^;];%d",
...
int le_vetor_veiculo_txt(int operacao){
    ...
    for(int i=0;i<vetor_tamanhos[2][1];i++){
        fgets(linha, sizeof(linha), p_arquivo);
        linha[strcspn(linha, "\n")] = '\0';
        if(sscanf(linha, "%9[^;];%29[^;];%29[^;];%d;%19[^;];%d;%d",
...
int le_vetor_peca_txt(int operacao){
    ...
    for(int i=0;i<vetor_tamanhos[3][1];i++){
        fgets(linha, sizeof(linha), p_arquivo);
        linha[strcspn(linha, "\n")] = '\0';
        if(sscanf(linha, "%d;%99[^;];%49[^;];%49[^;];%f;%f;%d;%d;%d",
        &vetor_peca[i].codigo,
...
int le_vetor_fornecedor_txt(int operacao){
    ...
    for(int i=0;i<vetor_tamanhos[4][1];i++){
        fgets(linha, sizeof(linha), p_arquivo);
        linha[strcspn(linha, "\n")] = '\0';
        if(sscanf(linha, "%d;%99[^;];%99[^;];%19[^;];%ld;%2[^;];%29[^;];%d;%29[^;];%49[^;];%d;%ld;%49[^;];%d",
...
void define_codigo_cliente(int posicao){
    vetor_cliente[posicao].codigo=posicao;
    return;
}

void define_codigo_peca(int posicao){
    vetor_peca[posicao].codigo=posicao;
    return;
}

void define_codigo_fornecedor(int posicao){
    vetor_fornecedor[posicao].codigo=posicao;
    return;
}

void define_codigo_servico(int posicao){
    vetor_servico[posicao].codigo=posicao;
    return;
}

int define_codigo_ordemservico(int posicao){
    vetor_ordemservico[posicao].codigo=posicao;
    return;
}

int define_id_contaspagar(int posicao){
    caixa.contas_pagar[posicao].id=posicao;
}

void exclui_em_vetor_cliente(int posicao){
    vetor_cliente[posicao].existe=0;
    return;
}

void exclui_em_vetor_veiculo(int posicao){
    vetor_veiculo[posicao].existe=0;
    return;
}

void exclui_em_vetor_peca(int posicao){
    vetor_peca[posicao].existe=0;
    return;
}

void exclui_em_vetor_fornecedor(int posicao){
    vetor_fornecedor[posicao].existe=0;
    return;
}

void exclui_em_vetor_servico(int posicao){
    vetor_servico[posicao].existe=0;
    return;
}

void exclui_em_vetor_funcionario(int posicao){
    vetor_funcionario[posicao].existe=0;
    return;
}

void exclui_em_vetor_ordemservico(int posicao){
    vetor_ordemservico[posicao].existe=0;
    return;
}
...
void* verifica_aloca(int indice, void* p, size_t tamanho_tipo){
    if(vetor_tamanhos[indice][1]==vetor_tamanhos[indice][0]-1){
        p=realoca_vetor(p, vetor_tamanhos[indice][0]+10, tamanho_tipo, indice);
    }
    return p;
}
...
void relatorio_cadastro_basico(int cadastro,int filtro,char* linha_filtro){
    ...
    switch(cadastro){
        case 1:
            ...
            for(int i=0;i<vetor_tamanhos[1][1];i++){
                if(filtro==1){
                    fprintf(p_arquivo,"%d;%s;%ld;%s;%s;%d;%s;%s;%d;%ld;%s;%d\n", vetor_cliente[i].codigo, vetor_cliente[i].nome,vetor_cliente[i].cpf_cnpj, vetor_cliente[i].endereco.estado, vetor_cliente[i].endereco.cidade, vetor_cliente[i].endereco.cep, vetor_cliente[i].endereco.bairro, vetor_cliente[i].endereco.rua, vetor_cliente[i].endereco.numero, vetor_cliente[i].telefone, vetor_cliente[i].email, vetor_cliente[i].existe);
                }else if(filtro==2){
